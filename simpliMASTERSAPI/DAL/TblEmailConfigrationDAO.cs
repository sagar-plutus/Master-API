...
namespace ODLMWebAPI.DAL
{
    public class TblEmailConfigrationDAO : ITblEmailConfigrationDAO
    {
        ...
        {
            String sqlSelectQry = " SELECT * FROM [dimEmailConfigration]"; 
            return sqlSelectQry;
        }
        #endregion
        ...
        public TblEmailConfigrationTO SelectDimEmailConfigrationIsActive()
        {
            ...
            {
                conn.Open();
                cmdSelect.CommandText = "SELECT * FROM [tblEmailConfigration] WHERE isActive = 1";
                cmdSelect.Connection = conn;
                cmdSelect.CommandType = System.Data.CommandType.Text;
        ...
        public TblEmailConfigrationTO SelectDimEmailConfigrationIsActive(int locationId)
        {
            String sqlConnStr = _iConnectionString.GetConnectionString(Constants.CONNECTION_STRING);
            SqlConnection conn = new SqlConnection(sqlConnStr);
            SqlCommand cmdSelect = new SqlCommand();
            ...
            {
                conn.Open();
                cmdSelect.CommandText = SqlSelectQuery() + " WHERE isActive = 1 And locationid="+ locationId +" ";
                cmdSelect.Connection = conn;
                cmdSelect.CommandType = System.Data.CommandType.Text;
        ...
        {
            String sqlQuery = @" INSERT INTO [dimEmailConfigration]( " + 
            " [portNumber]" +
            " ,[isActive]" +
            " ,[emailId]" +
            " ,[userName]" +
...
" VALUES (" +
            ...
            cmdInsert.Parameters.Add("@Host", System.Data.SqlDbType.VarChar).Value = dimEmailConfigrationTO.Host;
        ...
        {
            String sqlQuery = @" UPDATE [dimEmailConfigration] SET " + 
            ...
            cmdUpdate.Parameters.Add("@Host", System.Data.SqlDbType.VarChar).Value = dimEmailConfigrationTO.Host;
        ...
        public List<TblEmailConfigrationTO> ConvertDTToList(SqlDataReader dimEmailConfigrationTODT)
        {
            ...
            if (dimEmailConfigrationTODT != null)
            {
                while (dimEmailConfigrationTODT.Read())
                {
                    ...
                    if (HasColumn(dimEmailConfigrationTODT, "password") && dimEmailConfigrationTODT["password"] != DBNull.Value)
                        dimEmailConfigrationTONew.Password = Convert.ToString(dimEmailConfigrationTODT["password"].ToString());
                    if (HasColumn(dimEmailConfigrationTODT, "host") && dimEmailConfigrationTODT["host"] != DBNull.Value)
                        dimEmailConfigrationTONew.Host = Convert.ToString(dimEmailConfigrationTODT["host"].ToString());
                    if (HasColumn(dimEmailConfigrationTODT, "locationId") && dimEmailConfigrationTODT["locationId"] != DBNull.Value)
                        dimEmailConfigrationTONew.locationId = Convert.ToInt16(dimEmailConfigrationTODT["locationId"].ToString());
