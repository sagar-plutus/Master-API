       ...
       public List<DropDownTO> GetAvailableTimeZones()
        {
            ...
            {
                conn.Open();
                cmdSelect.CommandText = "select * from dimTimeZone";
                cmdSelect.Connection = conn;
                cmdSelect.CommandType = System.Data.CommandType.Text;

                SqlDataReader sqlReader = cmdSelect.ExecuteReader(CommandBehavior.Default);
                List<DropDownTO> list = ConvertDTToListTimeZone(sqlReader);                
                sqlReader.Dispose();
                return list;
            }
            catch (Exception ex)
...
//Added By Gokul
        ...
        public List<DropDownTO> ConvertDTToListTimeZone(SqlDataReader tblConfigParamsTODT)
        {
            List<DropDownTO> tblConfigParamsTOList = new List<DropDownTO>();
            if (tblConfigParamsTODT != null)
            {
                while (tblConfigParamsTODT.Read())
                {
                    DropDownTO tblConfigParamsTONew = new DropDownTO();
                    if (tblConfigParamsTODT["offset"] != DBNull.Value)
                        tblConfigParamsTONew.MappedTxnId = tblConfigParamsTODT["offset"].ToString();
                    if (tblConfigParamsTODT["displayText"] != DBNull.Value)
                        tblConfigParamsTONew.Text = tblConfigParamsTODT["displayText"].ToString();
                    if (tblConfigParamsTODT["timeZone"] != DBNull.Value)
                        tblConfigParamsTONew.Code = tblConfigParamsTODT["timeZone"].ToString();
                    if (tblConfigParamsTODT["valueTxt"] != DBNull.Value)
                        tblConfigParamsTONew.Tag = tblConfigParamsTODT["valueTxt"].ToString();
