...
namespace ODLMWebAPI.DAL
{
    public class TblAlertInstanceDAO : ITblAlertInstanceDAO
    {
        ...
        public int ExecuteInsertionCommand(TblAlertInstanceTO tblAlertInstanceTO, SqlCommand cmdInsert)
        {
            ...
            cmdInsert.Parameters.Add("@EmailComment", System.Data.SqlDbType.NVarChar).Value = Constants.GetSqlDataValueNullForBaseValue(tblAlertInstanceTO.EmailComment);
        ...
        public int DeactivateChatNotification(String alertDefinitionId, String sourceEntityId)
        {
            String sqlConnStr = _iConnectionString.GetConnectionString(Constants.CONNECTION_STRING);
            SqlConnection conn = new SqlConnection(sqlConnStr);
            SqlCommand cmdUpdate = new SqlCommand(); 
            try
            {
                conn.Open();
                cmdUpdate.Connection = conn;
                String sqlQuery = "update tblAlertInstance set isActive = 0 where alertDefinitionId IN(" + alertDefinitionId + ")";
                if (!String.IsNullOrEmpty(sourceEntityId))
                {
                    sqlQuery += " and sourceEntityId IN(" + sourceEntityId + ")";
                }
                cmdUpdate.CommandText = sqlQuery;
                cmdUpdate.CommandType = System.Data.CommandType.Text;
                return cmdUpdate.ExecuteNonQuery();
