    ...
    public class TblPagesDAO : ITblPagesDAO
    {
        ...
        public List<TblPagesTO> SelectAllTblPages(int moduleId)
        {
            ...
            try
            {
                conn.Open();
                if (moduleId > 0)
                    cmdSelect.CommandText = SqlSelectQuery() + " WHERE isActive=1 and moduleId=" + moduleId;
                else
        ...
        public TblPagesTO SelectTblPages(Int32 idPage)
        {
            ...
            {
                conn.Open();
                cmdSelect.CommandText = SqlSelectQuery()+ " WHERE isActive=1 and idPage = " + idPage +" ";
                cmdSelect.Connection = conn;
                cmdSelect.CommandType = System.Data.CommandType.Text;
        ...
        public List<TblPagesTO> ConvertDTToList(SqlDataReader tblPagesTODT)
        {
            ...
            if (tblPagesTODT != null)
            {
                while (tblPagesTODT.Read())
                {
                    ...
                    if (tblPagesTODT["isActive"] != DBNull.Value)
                        tblPagesTONew.IsActive = Convert.ToInt32(tblPagesTODT["isActive"].ToString());
